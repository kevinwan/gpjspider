...
def pagenum_function(url):
    """
    增量爬取获取页号
    http://quanguo.58.com/ershouche/pn7/?PGTID=1429...
    """
    if 'quanguo.58.com/ershouche/pn' not in url:
        return 1
    else:
        idx = url.find('quanguo.58.com/ershouche/pn')
        if idx < 0:
            return 99999999
        else:
            idx += len('quanguo.58.com/ershouche/pn')
            end_idx = url.find('/', idx)
            return int(url[idx:end_idx])
